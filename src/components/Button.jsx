...
export const Button = (props) => {
    ...
    if (props.btnClassesOnly) {
        return [{btn_class: btn_class, btn_container_class: btn_container_class }];
    }
    if (props.imgOnly) {
        return (
    ...
    if (!props.imgOnly &&  !props.btnClassesOnly) {
        return (
            <div className={btn_container_class}>
                {props.link ? <Link to={props.link} className={btn_class}>{props.text}</Link> : null}
                {props.href ? <a href={props.href} className={btn_class}>{props.text}</a> : null}
                {(!props.href && !props.link)  ? <p className={btn_class}>{props.text}</p> : null}
                <svg className="bg" width="90%" height="100%" viewBox="0 0 325 70" preserveAspectRatio="none">
                    <g id="Group_284" data-name="Group 284" transform="translate(-292.329 -685.757)">
                        <g id="Group_210" data-name="Group 210" transform="translate(346.285 688.757)">
                        <g transform="matrix(1, 0, 0, 1, -53.96, -3)">
